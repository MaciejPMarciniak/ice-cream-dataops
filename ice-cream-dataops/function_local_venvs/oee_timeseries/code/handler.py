...
def handle(client: CogniteClient, data: Dict[str, Any] = {}) -> None:
    lookback_minutes = (
        timedelta(minutes=data.get("lookback_minutes", 6000)).total_seconds() * 1000
    )
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(process_site, client, data_set_id, lookback_minutes, site)
            for site in sites
        ]
        for f in futures:
            f.result()
...
def process_site(client, data_set_id, lookback_minutes, site):
    ...
    timeseries = client.time_series.list(
        asset_subtree_external_ids=site.lower(), limit=None
    )
    ...
    assets_dps = {
        asset.external_id: [
            ...
            if asset.external_id in latest_dp.external_id
        ]
        for asset in assets
    }
    ...
    for asset, latest_dps in assets_dps.items():
        ...
        if end:
            dps_df: pd.DataFrame = client.time_series.data.retrieve_dataframe(
            ...
            dps_df[f"{asset}:planned_status"] = dps_df[
                f"{asset}:planned_status"
            ].fillna(value=backfill_value)

            # Same for status
            first_valid_value = dps_df[f"{asset}:status"].loc[
                dps_df[f"{asset}:status"].first_valid_index()
            ]
            backfill_value = 1.0 if first_valid_value == 0.0 else 0.0
            dps_df[f"{asset}:status"] = dps_df[f"{asset}:status"].fillna(
            ...
            dps_df[f"{asset}:oee"] = (
                dps_df[f"{asset}:quality"]
                * dps_df[f"{asset}:performance"]
                * dps_df[f"{asset}:availability"]
            )
            ...
            dps_df = dps_df.drop(
                columns=[
                    f"{asset}:{postfix}"
                    for postfix in ["good", "count", "status", "planned_status"]
                ]
            )
