...
def get_timeseries_for_site(client: CogniteClient, site: str, config: Config):
        ...
        for ts in time_series:
            ...
            if not config.extractor.backfill:
                latest = (
                    ...
                    if latest_dps.get(ts.external_id)
                    else None
                )
                start = latest if latest else now - increment
            else:
                start = (
                    now - timedelta(hours=config.extractor.hours).total_seconds() * 1000
                )
            end = now

            dps = ice_cream_api.get_datapoints(
            ...
            for external_id, datapoints in dps.items():
                upload_queue.add_to_upload_queue(
                    external_id=external_id, datapoints=datapoints
                )
